...
'The Weeping Angels arent props – theyre played by actresses in stone costumes who have to stay very, very still.',
'The noise the TARDIS makes was created by rubbing piano strings with a key.',
'Nicholas Briggs, the voice of the Daleks in recent series, had to bring along his own analogue voice modulator to create the voice as the BBC couldnt find a way to replicate their distinctive sound digitally.',
'In an early draft of the very first Doctor Who script, its strongly implied that the Doctor is planning to kill future companions Ian and Barbara in order to protect his secret. This was quickly taken out.',
'Wendy Padbury, the agent who discovered Matt Smith as a young actor, played the second Doctors companion Zoe Heriot in the late 1960s.',
'In fact, three of the actresses who played the Doctors companions went on to become agents, and all of them ended up representing actors who would play the Doctor.',
'In 1988, Paramount Pictures wanted to make a Doctor Who movie starring either Michael Jackson or Bill Cosby as the Doctor.',
'The first Doctor Who novel had the unwieldy title Doctor Who in an Exciting Adventure With the Daleks.',
'The name "Time Lord" to describe the Doctors people wasnt used until 1969, five-and-a-half years after the show started. It was another four-and-a-half years before his home planet was named on screen as "Gallifrey".',
'Shouty actor Brian Blessed claims he was offered the role of the Second Doctor (which eventually went to Patrick Troughton), but turned it down.',
...
'Peter Capaldi used to be in a punk band with Late Late Show host Craig Ferguson.',
'The Doctor has been married three times (that we know about) – to Queen Elizabeth I, Marilyn Monroe, and River Song.',
'Tennant is married to Georgia Moffett, who played the Doctors daughter opposite him in "The Doctors Daughter" and is actually the daughter of the fifth Doctor, Peter Davison. So the couples children have Doctors for their father and grandfather.',
'Tom Bakers famously long scarf as the fourth Doctor was created partly by accident. The knitter hired to create it, Begonia Pope, wasnt told how long it needed to be, so just used up all the wool she was given. It ended up 20 feet long (but was eventually trimmed down a bit).',
'In 2009 a group of volunteers clearing a pond in Hampshire found a submerged Dalek. Its not clear how it got there.',
'The gas masks in "The Empty Child" were made in part from cans of baked beans after attempts to find authentic WWII gas masks failed because they contain dangerous asbestos.',
...
'The 2008 episode "The Fires of Pompeii" was notable for having a future companion and a future Doctor in its cast as different characters – Karen Gillan and Capaldi.',
'Peter Hawkins, the original voice of the Daleks, went on to voice the character of Zippy in childrens TV show Rainbow. He was replaced as Zippy after the first series by Roy Skelton – who had also replaced him as the voice of the Daleks.',
...
'A 1966 internal BBC memo about the Doctors first regeneration suggested it was supposed to be like a really bad LSD trip.',
'One of the earliest story ideas for Doctor Who was a version of Dickens A Christmas Carol, but with a "slightly tipsy" Doctor as Jacob Marley. (The series finally did a loose adaptation of A Christmas Carol in 2010, but with added sharks.)',
'Verity Lambert, Doctor Whos first producer, was both the youngest producer in the BBCs drama department and the only woman in such a role.',
'In a previous job on ITVs Armchair Theatre series in 1958, Lambert – then just a production assistant – had to unexpectedly step in to studio-direct a live broadcast of a play after one of the actors died during transmission.',
'A teenage Capaldi had his Doctor Who fan art published in Doctor Who International Fan Club Magazine.',
'The young Capaldi was so desperate to be put in charge of the Official Doctor Who Fan Club that he bombarded the BBC with letters to the point where the producers secretary said: "I wish the Daleks or someone would exterminate him."',
'In 2007, Hugh Grant told The Sun that hed been offered the role of the Doctor but turned it down because he didnt think the new show would be a success.',
'The person who was originally supposed to design the Daleks was Ridley Scott – later the director of Alien and Gladiator – but he left his job at the BBC before he could get around to it.',
'An audience research report from 1988 showed that Bonnie Langfords character, Mel, was "unpopular" (to say the least) with viewers – "56% of respondents who answered a questionnaire on the Paradise Towers story wished she had been eaten".',
'The 1965 episode "Mission to the Unknown" is the only one in the series history that doesnt feature the Doctor, the TARDIS, or any of his companions.',
'Alternative costume designs for Smith included one design that was described as being like a pirate.',
'The Daleks might never have existed if comedian Tony Hancock hadnt had a massive row with his then scriptwriter, Terry Nation. Nation was about to decline an offer to write the second Doctor Who story, but changed his mind when Hancock sacked him that night – luckily, before his agent had had a chance to turn the BBC down.',
'Doctor Whos main creator, Sydney Newman, was furious about the inclusion of the Daleks, as hed been adamant that he didnt want clichéd "bug-eyed monsters" in his show. (He later admitted he might have been wrong about this.)',
'The fourth Doctor advertised computers on televison.',
'The words "Dalek" and "TARDIS" have become so widely used that they now both appear in the Oxford English Dictionary.',
'Its not entirely clear what the Doctor is a doctor of. Over the years, hes both claimed that he is a medical doctor and also denied it.',
'It took six years to trademark the TARDIS.',
    ...
    if (message.substring(0, 1) == '!') {
        ...
        switch(cmd) {
            ...
            case 'fact':
                ...
                var message = 'Doctor Who Fact #' + (random +1) + "  - " +  factArray[random];
                bot.sendMessage({
                    to: channelID,
                    message: message
                });
                logger.info('Executed: Fact');
            break;
            ...
            case 'ymama':
                ...
                logger.info('Executed: yomama!');
            break;

            // !yomamadw
            case 'ymamadw':
                //generate a random fact from the fact array and send to the channel as a message
                var random = Math.floor(Math.random()*yomamadw.length);
                var message = yomamadw[random];
                bot.sendMessage({
                    to: channelID,
                    message: message
                });
